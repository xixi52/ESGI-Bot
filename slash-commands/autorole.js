            ...
            if (results[0]) {
              if(results[0].status == "0") {
                if(choiceInt == 0) {
                  ...
                  database.query(
                    "UPDATE `autorole_channel` SET `status` = '" + choiceInt.toString() + "' WHERE `channel` = '" + channelID + "'",
                    async function (error, results, fields) {
                  });
                  await interaction.reply({
                    content:
              ...
              database.query(
                "INSERT INTO `autorole_channel`(`channel`, `status`) VALUES ('" + channelID + "','" + choiceInt.toString() + "')",
                async function (error, results, fields) {});
              if(choiceInt == 0) {
                await interaction.reply({
                  content:
